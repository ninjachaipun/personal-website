...
function drawLineChart(containerId, opts) {
  ...
  opts.series.forEach((s) => {
    ...
    const endLabel = svgEl('text', {
      ...
      class: 'value-label', fill: s.color, 'font-weight': '700'
    });
...
function drawStackedBar(containerId, opts) {
  ...
  opts.segments.forEach((s) => {
    ...
    if (segW > 60) {
      const label = svgEl('text', { x: x + segW / 2, y: barH / 2 + 5, class: 'value-label', 'text-anchor': 'middle', fill: '#ffffff', 'font-weight': '700' });
